...
func TestManager_off(t *testing.T) {
	ch1 := &alertChannelMock{}
	ch1.On("SendSuccess", message.New("alert-name", "alert-text")).Return(nil)

	ch2 := &alertChannelMock{}
	ch2.On("SendSuccess", message.New("alert-name", "alert-text")).Return(nil)
	...
	ch1.AssertCalled(t, "SendSuccess", mock.Anything)
	ch2.AssertCalled(t, "SendSuccess", mock.Anything)
	ch1.AssertExpectations(t)
}
